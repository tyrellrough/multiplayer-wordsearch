...
class WSRendererMultiplayer {
    ...
    constructor(canvas, context, numBoardRows) {
        ...
        console.log("numboardrows: " + numBoardRows);
    ...
    #drawLetters(cellArray) {
        for(let i = 0; i < cellArray.length; i++) {
            ...
            this.#drawChar(yCP, xCP, cellArray[i].character, this.#fontSize);
        }
    }
    ...
    highlightCellWithoutColParam(cell) {
        const index2D = ArrayHelperFunctions.convert1DIndexTo2D(cell.cellNum1D, this.#numBoardRows, this.#numBoardRows);
        const cellYStart = index2D[0] * this.#boardCellHeight;
        const cellXStart = index2D[1] * this.#boardCellHeight;
        const cellYCenter = cellYStart + 0.5 * this.#boardCellHeight;
        const cellXCenter = cellXStart + 0.5 * this.#boardCellHeight;
        ...
        //get cell colour
        this.#canvas.getContext('2d').fillStyle = `${cell.color}`;
        this.#canvas.getContext('2d').fill();
    }

    highlightCells(cellArray) {
        for(let i = 0; i < cellArray.length; i++) {
            if(cellArray[i].color !== "") {
                this.highlightCellWithoutColParam(cellArray[i]);
            }
        }
    }
}

export default WSRendererMultiplayer;